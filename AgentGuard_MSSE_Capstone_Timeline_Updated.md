...
## Recommended Database Evolution Schedule

The database architecture will evolve deliberately across the project rather than being changed during the final stabilization period.

| Sprint | Database plan |
|---|---|
| Sprint 1 | SQLite is acceptable for the initial prototype and basic feature demonstration. |
| Sprint 2 | Migrate to PostgreSQL and add Alembic migrations, relational constraints, indexes, and repeatable seed scripts. |
| Sprint 3 | Build the approval workflow, MCP/agent integration, and audit features using PostgreSQL. |
| Sprint 4 / Final Sprint | Perform testing, optimization, security hardening, deployment, and documentation without changing the main database architecture. |

**Architecture decision:** SQLite is used only to reduce setup complexity during the first prototype. PostgreSQL becomes the primary development, testing, and production database from Sprint 2 onward. Major database changes should be completed before Sprint 4 so that the final sprint can focus on stability and submission readiness.
